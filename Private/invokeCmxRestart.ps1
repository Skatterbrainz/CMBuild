function invokeCmxRestart {
	...
	.EXAMPLE
	invokeCmxRestart -XmlFile .\cmbuild.xml
	.NOTES
	#>
	param (
...
"@
	...
	writeLogFile -Category 'Info' -Message "PowerShell file... $psfile"
	$cmd = @"
`@echo off
...
"@
	...
	writeLogFile -Category 'Info' -Message "Batch file........ $batfile"

	try {
		Invoke-Expression -Command $stask -ErrorAction Stop
		writeLogFile -Category 'Info' -Message "Scheduled Task.... $stask"
	} catch {
